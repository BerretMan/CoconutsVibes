...
async def add(interaction: discord.Interaction, music: str):
    ...
    if file_name in os.listdir("Music/Youtube/"):
        ...
        ydl_opts = {
            ...
            "outtmpl": f"Music/Youtube/{video_id}@{video_title}.%(ext)s",
            "postprocessors": [
                {
...
async def goto(interaction: discord.Interaction, timecode: str):
    ...
    bot.vc.play(
        discord.FFmpegPCMAudio(
            f"Music/Youtube/{bot.current_music}", options=f"-ss {temps}"
        )
    )
    bot.isGoto = False
    ...
    await interaction.response.send_message(
        f"Avancement vers {minute_str}:{seconde_str}", ephemeral=True
    )
